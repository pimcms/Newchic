	...
	$.newchic_product = {
		...
		select_poa: function(o) {
			var htm = 	'<div class="goods_info">'+
							'<img src="http://img.newchic.com/thumb/other_items//upload/2015/05/SKU212498-1 (2).jpg">'+
							'<span class="title">SCULPT Premium Bandage Skater Dress</span>'+
			...
			$(document).off("click", ".button_yes").on("click", ".button_yes", function() {
				...
				if(!is_select){
					...
					attrs[380] = parseInt(parent_box.find('.goods_item_attr .size .active').attr('value_id'));
					var warehouse = 'CN';
					$.ajax({
						type: 'post',
						url: 'index.php?com=shopcart&t=addproduct',
...
function myFly() {	
	...
	var oHeight = $(".main").outerHeight();
	var oWidth = $(".main").outerWidth();
